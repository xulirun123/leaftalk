    ...
    <div class="settings-content">
      ...
      <div class="settings-section">
        ...
        >
          <div class="setting-info">
            <span class="yy-function-item-text">{{ option.label }}</span>
          </div>
      ...
      <div class="settings-section">
        ...
        <div class="setting-item" @click="manageMomentsBlacklist">
          <div class="setting-info">
            <span class="yy-function-item-text">选择朋友</span>
...
const loadFriends = async () => {
  try {
    ...
    const arr: any[] = Array.isArray(resp?.data) ? resp.data : (Array.isArray((resp as any)?.data?.data) ? (resp as any).data.data : [])
    const friends: Friend[] = (arr || []).map((c: any) => ({
      id: String(c.id || c.user_id || ''),
      name: String(c.nickname || c.name || c.yeyu_id || '好友'),
      avatar: c.avatar
    })).filter(f => f.id)
